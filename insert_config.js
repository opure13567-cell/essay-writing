...
const template = `你是一名普通大学本科生，正在完成一门课的作业。写一篇{{type}}，字数约{{words}}字。
...
async function main() {
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/config?key=in.(pricing_rules,payment_qr_wechat,payment_qr_alipay,ai_templates)`, {
    method: "DELETE",
  ...
  // Insert all config
  const configs = [
    { key: "pricing_rules", value: JSON.stringify([{maxWords:500,price:2},{maxWords:5000,price:5},{maxWords:100000,price:10}]) },
    { key: "payment_qr_wechat", value: JSON.stringify("https://bxipdwwzkgpajcmmmulq.supabase.co/storage/v1/object/public/uploads/wixinshoukuanma.jpg") },
    { key: "payment_qr_alipay", value: JSON.stringify("") },
    { key: "ai_templates", value: JSON.stringify([template]) },
  ];
  ...
  for (const cfg of configs) {
    ...
    console.log(`${cfg.key}: ${res.status} ${data.substring(0,100)}`);
  }
